...
## 抽出ワード
### 抽出ワード
ここで指定した単語がレスに含まれていたら、字幕に色をつける。「,」区切りで、複数指定可能。

### 着色
上で指定した単語が含まれたときの字幕の色。
...
### 着信音を鳴らす
チェックすると、上で指定した単語が含まれるレスが付いた時に着信音を鳴らす。
...
## NGワード
### NGワード
本文にこの単語が含まれるレスを非表示にする。「,」区切りで、複数指定可能。
...
# 本体以外の設定
画面取込しても字幕などが表示されていない場合は、SCFHで「レイヤードウィンドウ表示」をチェックして取り込むと映るかもしれません。

低ビットレートで配信している場合、いつもよりビットレートをあげておいた方が字幕がきれいに映ることがあるので、配信する前に試してみると良いと思います。

# 使用方法
設定が終ったら「START」をクリックすれば、開始です。少しするとレスが右から流れてくると思います。

右下の数字は、現在のレス番です。クリックすると色がつき、字幕の流れがストップします。もう一度クリックすると流れが再開します。

# 更新履歴
## ver 1.1
 -字幕のフォントを設定できるようにしました。
## ver 1.2
 -「ageたレスの字幕を下方に表示する」の設定を追加しました。
 -複数行のレスの際にごみが表示される場合があったのを修正しました。
